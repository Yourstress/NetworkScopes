...
public class ExampleClientLauncher : MonoBehaviour
{
	...
	void DrawLobbyGUI()
	{
		...
		using (new GUILayout.HorizontalScope())
		{
			...
			if (GUILayout.Button("Create"))
				client.Lobby.Server.CreateMatch(matchName);
		}
			...
			for (int x = 0; x < client.Lobby.availableMatches.Length; x++)
			{
				using (new GUILayout.HorizontalScope())
				{
					...
					if (GUILayout.Button("Join"))
						client.Lobby.Server.JoinMatch(client.Lobby.availableMatches[x]);
				}
			}
		}
	}
	...
	void DrawMatchGUI()
	{
		...
		if (GUILayout.Button("Send Auto-Serialized Object"))
		{
			...
			client.Match.Server.TestAutoSerializedObj(obj);
		}
		if (GUILayout.Button("Leave Match"))
		{
			client.Match.Server.LeaveMatch();
		}
	}
}